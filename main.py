...
# print (mirror(1252563983458972345))
##########################################################################################
...
##########################################################################################
# Для вычисления числа e с определенной точностью можно использовать формулу: e_n = (1 + 1/n)**n
...
##########################################################################################
# И в качестве актуализации темы итераторов давайте вспомним, какого вида исключение появляется при попытке выйти за пределы итерируемого объекта. Чтобы вспомнить, можно запустить следующий код:
...
# При согласии пользователя на авториз
# ацию ему предлагается ввести username,
# который также хранится в глобальной области видимости.
# Функция должна использовать два декоратора: один для проверки авторизации
...
#         else:
#             print("Доступ для", username, " запрещен")
#     return wrapper
#
# yesno = input(
...
# map + filter
# some_list = [i - 10 for i in range(21)]
